<template lang="html">
  <div class="realtime-wrapper group">
    <div class="sessions-list">
      ...
      <ul>
        <li class="current" v-for="session in sessions">
          ...
          <div class="user-status">
            {{session.status}} <span class="time">{{sessionTime(session.time)}}</span>
          </div>
        </li>
      </ul>
    </div>
...
export default {
  mounted () {
    const socket = io('http://192.168.111.92:3000')

    if (notify.default.needsPermission) {
      notify.default.requestPermission()
    }

    socket.on('new-session', (sessionData) => {
      this.pushSession({
        id: '122',
